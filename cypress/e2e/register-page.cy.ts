describe('Tests in Register Page', () => {
    ...
    describe('Submit the account registration form', () => {
        ...
        it('should show error messages when the form is submitted empty', () => {
            ...
            cy.getByTestId('companyName-error').should('contain.text', 'O nome da empresa deve ser preenchido.');
            cy.getByTestId('contactNumber-error').should('contain.text', 'O número de telefone deve ser preenchido.');
            cy.getByTestId('CNPJorCPF-error').should('contain.text', 'Um CNPJ ou um CPF deve ser informado.');
        ...
        it('should show an error when the name has less than 3 characters', () => {
            ...
            cy.getByTestId('register-button').should('be.visible').click();
            cy.getByTestId('name-error').should('contain.text', 'O nome deve conter 3 ou mais caracteres.');
        });
        ...
        it('should show an error when contact number is too short', () => {
            ...
            cy.getByTestId('register-button').should('be.visible').click();
            cy.getByTestId('companyName-error').should('contain.text', 'O nome da empresa deve conter 3 ou mais caracteres.');
        });

        it('should show an error when CNPJ or CPF is empty', () => {
            cy.getByTestId('contactNumber-input').should('be.visible').type('(48) 99999-99');
            cy.getByTestId('register-button').should('be.visible').click();
            cy.getByTestId('contactNumber-error').should('contain.text', 'O número de telefone deve conter no mínimo 14 caracteres.');
        });
        ...
        it('You must check if the password view icon is working', () => {
            ...
            cy.getByTestId('password-input').should('have.attr', 'type', 'text').and('have.value', '12345678');
            cy.getByTestId('view-password').click({ multiple: true });
        ...
        it('password must be 8 characters or more', () => {
            ...
            cy.getByTestId('register-button').should('be.visible').click();
            cy.getByTestId('confirmPassword-error').should('contain.text', 'Sua senha deve possuir 8 ou mais caracteres.');
        });
