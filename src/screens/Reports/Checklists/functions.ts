...
export const requestReportsData = async ({
  ...
  setMaintenances([]);
  await Api.get(
    `/buildings/reports/list?maintenanceStatusIds=${filters.maintenanceStatusIds}&buildingIds=${filters.buildingIds}&startDate=${filters.startDate}&endDate=${filters.endDate}`,
  )
    .then((res) => {
