...
// SERVICES
import { getStockHistoryActivities } from '@services/apis/getStockHistoryActivities';
import { postStockHistoryActivity } from '@services/apis/postStockHistoryActivity';
...
interface IStockHistoryActivities {
  stockId: string;
  userId?: string;
  disableComment?: boolean;
}

export const StockHistoryActivities = ({
  stockId,
  userId,
  disableComment = false,
}: IStockHistoryActivities) => {
  const [activities, setActivities] = useState<IStockActivity[]>([]);
  const [filteredActivities, setFilteredActivities] = useState<IStockActivity[]>([]);
  ...
  const handleGetStockHistoryActivities = async () => {
    setLoading(true);

    try {
      const responseData = await getStockHistoryActivities(stockId);

      setActivities(responseData.stockActivities);
    } catch (error: any) {
  ...
  const handleCreateOneStockHistoryActivity = async () => {
    setLoading(true);

    try {
      await postStockHistoryActivity({
        stockId,
        userId,
        activityContent,
      ...
      setActivityContent('');
      setImagesToUpload([]);
      handleGetStockHistoryActivities();
    } catch (error: any) {
      handleToastify(error);
    } finally {
      setLoading(false);
          ...
          <Style.InputRow>
            ...
            <Style.InputButtons>
              ...
              <IconButton
                ...
                permToCheck="stock:update"
                onClick={() => handleCreateOneStockHistoryActivity()}
              />
            </Style.InputButtons>
          </Style.InputRow>
